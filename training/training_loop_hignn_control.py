...
def training_loop(
    ...
    network_kwargs      = dict(class_name='training.networks_edm2_hignn_control.Precond'),
    loss_kwargs         = dict(class_name='training.training_loop_hignn_control.EDM2Loss'),
    optimizer_kwargs    = dict(class_name='torch.optim.Adam', betas=(0.9, 0.99)),
    lr_kwargs           = dict(func_name='training.training_loop_hignn_control.learning_rate_schedule'),
    ...
    while True:
        ...
        if caption_dropout != 0.0:
            misc.set_random_seed(seed, state.cur_nimg)
            apply_cfg = np.random.rand() < caption_dropout
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                image_latents = encoder.encode_latents(graph_batch.image.to(device))
                graph_batch.image = image_latents
                graph_batch.caption = None if apply_cfg else graph_batch.caption
