...
class UNet(torch.nn.Module):
    ...
    def forward(self, x, noise_labels, graph, class_labels):
        ...
        if self.emb_label is not None:
            emb = mp_sum(emb, self.emb_label(class_labels / np.sqrt(self.class_labels_var)), t=self.label_balance)
        emb = mp_silu(emb)
        ...
        for name, block in self.dec.items():
            if 'block' in name:
                x = mp_cat(x, mp_sum(skips.pop(), self.control_d[name](control_skips.pop())), t=self.concat_balance)
